    ...
    async def prepare(
        ...
        *,
        ...
        if runtime_epoch != self.config.epoch:
            self.config = ExtraPrefixCacheConfig.from_any({**self.config.__dict__, "runtime_model_cache_epoch": runtime_epoch})
        ...
        if self.config.warmup and warmup_write_enabled(self.config.write_policy) and cache_salt not in self._warmed_cache_salts:
