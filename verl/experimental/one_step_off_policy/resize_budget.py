...
class ResizeBudgetConfig:
    ...
    def from_dict(cls, cfg: dict | None) -> "ResizeBudgetConfig":
        ...
        return cls(enable=bool(cfg.get("enable", False)), memory_budget_ratio=min(max(ratio, 0.1), 0.99))
...
class ResizeBudgetController:
    """Conservative budget gate driven by one global memory budget ratio."""
    ...
    def evaluate_restore(
        self, *, snapshot: ResizeBudgetSnapshot, estimated_gpu_peak_bytes: int
    ) -> ResizeBudgetDecision:
        if not self.config.enable:
