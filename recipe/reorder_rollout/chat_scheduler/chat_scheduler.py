...
class ReorderScheduler(MicroBatchScheduler, ReorderSchedulerMixin):
    def __init__(
        ...
        self.rollout_req_handler = rollout_req_handler if rollout_req_handler else self.reorder_handle_rollout_req
    ...
    async def reorder_handle_rollout_req(
        self,
        server_handle,
        ...
        try:
            ...
            for ids, mgr in mgr_dict.items():
                if mgr.ray_awaitable is not None:
                    ...
                    maybe_resp: RequestOutput = await mgr.server_handle.abort_request.remote(request_id)
